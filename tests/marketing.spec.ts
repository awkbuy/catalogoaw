...
test.describe("Marketing - campos por juego", () => {
  ...
  test("crear y editar un juego persisten los campos de marketing", async ({ page }) => {
    ...
    expect(created.googleProductCategory).toBe("Toys & Games > Games > Board Games");
    expect(created.metaProductCategory).toBe("Juegos de mesa");
    expect(created.gtin).toBe("0702217114061");
    expect(created.mpn).toBe("WR-TEST-001");
    expect(created.brand).toBe("Wolfie Room");
    expect(created.condition).toBe("new");
    expect(created.marketingPriority).toBe(5);

    const updateRes = await page.request.put(`/api/admin/juegos/${created.id}`, {
      data: {
        nombre: "Juego Marketing Test",
        slug,
        descripcion: "Test de campos de marketing",
