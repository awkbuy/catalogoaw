...
function Dock({
  ...
  useMotionValueEvent(isHovered, 'change', (v) => {
    hoveredRef.current = v;
  });

  useAnimationFrame(() => {
    const el = scrollRef.current;
    if (!el || hoveredRef.current !== 1) return;
    ...
    if (clientX < rect.left + threshold) {
      delta = -(((rect.left + threshold - clientX) / threshold) * speed);
    } else if (clientX > rect.right - threshold) {
      delta = ((clientX - (rect.right - threshold)) / threshold) * speed;
    }
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          'mx-auto flex w-fit gap-4 rounded-2xl bg-gray-50',
          className
        )}
        style={{
          height: panelHeight,
          paddingLeft: Math.round(magnification / 2),
          paddingRight: Math.round(magnification / 2),
        }}
        role='toolbar'
